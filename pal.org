...
#+begin_src latex
  ...
  or subject), their lengths, and the alignment score. Once we've written
  them, we flush the \ty{tabwriter}.
#+end_src
...
#+end_src
#+begin_src latex
  We declare \ty{score} as an alignment field.
#+end_src
#+begin_src go <<Alignment fields>>=
  score float64
#+end_src
#+begin_src latex
  ...
  For this we need access to the aligned sequences and the length of an
  alignment line.
#+end_src
#+begin_src go <<Alignment fields>>=
  qa, sa []byte
...
#+end_src
#+begin_src latex
  We initialize the query and subject alignments, and the line length.
#+end_src
#+begin_src go <<Construct alignment>>=
  a.qa = make([]byte, 0)
  a.sa = make([]byte, 0)
...
#+begin_src latex
  ...
  !the actual alignment, invoke the method \ty{Align}.
...
#+end_src
#+begin_src latex
  The last step of the trace back is to implement the function
  \ty{reverse} to reestablish the left-to right ordering of our
  alignment. In the loop we move simultaneously from the start and the
  end of the slice to the middle swapping a pair of residues at each
...
#+begin_src latex
  ...
  over the first three of these terms, we can use it here.
#+end_src
...
#+begin_src latex
  ...
  structure we shall use later on to store all possible starting points,
  a slice of coordinates. Each coordinate consists of a pair of indexes
  and a score.
#+end_src
#+begin_src go <<Types>>=
