...
#+end_src
#+begin_src go <<Construct residue string>>=
...
#+end_src
#+begin_src go <<Construct byte string>>=
  nr := 256
  res := make([]byte, nr)
  for i := 0; i < nr; i++ {
	  res[i] = byte(i)
...
#+end_src
#+begin_src go <<Determine back pointer>>=
  if v == a.p[i][j].E {
	  c = '^'
  } else if v == a.p[i][j].F {
	  c = '<'
...
#+end_src
#+begin_src go <<Determine cell entry>>=
  if t == 'e' {
	  v = a.p[i][j].E
  } else if t == 'f' {
	  v = a.p[i][j].F
...
#+end_src
#+begin_src latex
  In preparation of all subsequent alignments, we erase the starting
  points of the previous alignment, store all possible alignment
  starting points of the new alignments, and sort them.
#+end_src
...
#+end_src
#+begin_src latex
  We reset the alignments. Then we iterate over the coordinates and run
  a trace back from each one. If the trace back was successful, i. e. we
  found an alignment, we lop off the coordinates we've used up and break
  from the loop.
#+end_src
...
#+end_src
#+begin_src latex
  We reset the alignment strings to empty.
#+end_src
