...
#+begin_src latex
  ...
  sequences add one more, \ty{U} for uracil. We go through the
  characters that can appear in molecular sequence and set the
...
#+begin_src latex
  ...
  ! dynamic programming matrix. It takes as argument the type of
  ! printing desired, v, e, f, g for the cell elements, and t for trace
  ! back.
#+end_src
#+begin_src go <<Methods>>=
...
#+end_src
#+begin_src latex
  We extract the query and subject individually from the raw alignment sequences.
#+end_src
#+begin_src go <<Extract query and subject sequences>>=
  //<<Extract query>>
  //<<Extract subject>>
#+end_src
#+begin_src latex
  We extract the query by squeezing out the gaps.
#+end_src
#+begin_src go <<Extract query>>=
  i := 0
  for _, c := range q {
	  if c != '-' {
		  q[i] = c
...
#+end_src
#+begin_src latex
  We apply the same reslicing to extract the subject.
#+end_src
#+begin_src go <<Extract subject>>=
  i = 0
  for _, c := range s {
	  if c != '-' {
		  s[i] = c
...
#+end_src
#+begin_src go <<Print matrix header>>=
  fmt.Fprintf(w, "\t-")
  for _, c := range s {
	  fmt.Fprintf(w, "\t%c", c)
  }
...
#+begin_src latex
  ...
  requested cell entry.
#+end_src
#+begin_src go <<Print first row of matrix>>=
  m := len(q)
  n := len(s)
  fmt.Fprint(w, "-")
  for j := 0; j <= n; j++ {
...
#+end_src
#+begin_src go <<Print rest of matrix>>=
  for i := 1; i <= m; i++ {
...
#+begin_src latex
  ...
  with a plotting tool. 
#+end_src
#+begin_src go <<Print trace back>>=
  var i, j int
  buf := new(bytes.Buffer)
...
#+end_src
#+begin_src latex
  We'd like to observe the convention that alignments start in the top
  left hand corner, rather than the bottom left hand corner of
  graphs. We do this by setting the y coordinates negative.
#+end_src
#+begin_src go <<Print x/y  coordinates>>=
  for p, c := range q {
	  fmt.Fprintf(buf, "%d %d\n", j, -i)
	  if c == '-' {
		  i++
